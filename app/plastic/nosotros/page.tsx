...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.34em] text-slate-400">
      <span className="h-px w-8 bg-[#6b7280]" />
      {children}
    </p>
  );
}

export default async function PlasticNosotrosPage() {
  ...
  return (
    <main className="min-h-screen bg-white text-slate-950">
      ...
      <section className="relative overflow-hidden border-b border-slate-200">
        <div
          className="pointer-events-none absolute -right-40 -top-40 h-[420px] w-[420px] rounded-full bg-[#6b7280]/[0.07] blur-3xl"
          aria-hidden="true"
        />
        <div className="relative mx-auto max-w-[1632px] px-5 py-20 md:px-8">
          <ul className="grid gap-y-14 md:grid-cols-3 md:gap-x-14 md:gap-y-0">
            {pilares.map((pilar, index) => (
              ...
              >
                <h2 className="text-lg font-bold uppercase tracking-[0.06em] text-[#6b7280]">
                  {pilar.title}
                </h2>
      ...
      <section className="relative overflow-hidden border-b border-slate-200">
        <div
          className="pointer-events-none absolute -left-32 bottom-0 h-[380px] w-[380px] rounded-full bg-[#6b7280]/[0.06] blur-3xl"
          aria-hidden="true"
        />
        <div className="relative mx-auto max-w-[1632px] px-5 py-20 md:px-8">
            ...
            {valores.map((valor, index) => (
              <li key={valor.title} className="group border-t border-slate-200 pt-6">
                <div className="flex items-center gap-3">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[#6b7280]/25 bg-[#6b7280]/[0.06] font-mono text-[10px] font-medium tabular-nums text-[#6b7280]">
                ...
                <span className="mt-4 block h-px w-0 bg-[#6b7280] transition-all duration-500 ease-out group-hover:w-full" />
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="border-b border-slate-200">
        <div className="mx-auto max-w-[1632px] px-5 py-20 md:px-8">
          ...
          <p className="mt-8 max-w-2xl border-l border-[#6b7280]/40 pl-8 text-xl font-normal leading-9 tracking-[-0.005em] text-slate-700 md:text-2xl">
            {t("plastic-filosofia-texto")}
          </p>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-[#0b0c0d]">
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, rgba(11,12,13,0.98) 0%, rgba(24,25,27,0.9) 55%, rgba(107,114,128,0.3) 100%), radial-gradient(circle at 15% 80%, rgba(107,114,128,0.32), transparent 36%)",
          }}
        />
